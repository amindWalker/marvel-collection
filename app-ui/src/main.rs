...
fn App(cx: Scope) -> Element {
    let mut _api_data = Vec::<MarvelRoot>::with_capacity(1562 * 2);
    let data: Data = Default::default();
    let _api_state = use_state(cx, || data);

    // Get variables by matching them with injected .env
    let [API_BASE_URL, PUBLIC_KEY, HASH] = [
        env!("MARVEL_BASE_URL"),
        env!("MARVEL_PBK"),
        env!("MARVEL_API_HASH"),
    ];
    ...
    match fetch_api.value() {
        Some(Ok(comic)) => {
            rsx! {
                comic.data.results.clone().iter().map(|b|
                    rsx! { div {"{b.id}"} }
                )
            }
        },
        Some(Err(_)) => rsx! { p { "Error while loading the database." } },
        _ => rsx! { p { "Cannot connect to Marvel at this time." } }
    };

    cx.render(rsx! {
        main {
            class: "@apply main-container grid overflow-hidden",
            header {
                class: "fixed z2 w-full bg-black bg-opacity-50 backdrop-blur flex items-center justify-center drop-shadow-lg",
                div {
            ...
            body {
                ...
                section {
                    ...
                    nav {
                        class: "hidden md:grid col-span-1 z1 grid bg-red",
                        match fetch_api.value() {
                            Some(Ok(comic)) => {
                                rsx! {
                                    h2 {
                    ...
                    // bkacground blur using the comic image
                    // on mouse over brightness + animation focus
                    article {
                        class: "auto-cols-fr overflow-hidden",
                        match fetch_api.value() {
                            Some(Ok(_)) => rsx! { Card {} },
                            Some(Err(_)) => rsx! { p { "Error while loading the database." } },
                            _ => rsx! { p { "Can't connect to Marvel at this time." } }
